...
def raw_orders_row_count(context):
    count = duckdb.connect("local.duckdb").sql("SELECT count(*) FROM raw.orders").fetchone()[0]
...
def raw_fulfillment_row_count(context):
    count = duckdb.connect("local.duckdb").sql("SELECT count(*) FROM raw.fulfillment").fetchone()[0]
    return AssetCheckResult(
        passed=count > 0,
        severity=AssetCheckSeverity.ERROR,
        metadata={"row_count": count},
...
def stg_orders_no_null_pk(context):
    count = duckdb.connect("local.duckdb").sql(
